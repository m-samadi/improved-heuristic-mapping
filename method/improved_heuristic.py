...
#
# Perform an improved version of the heuristic task-to-thread mapping in
# OpenMP presented in the following paper:
...
def alloc_heuristic(num_threads, alloc_alg, t):
	global alloc_queue, exec_queue, curr_thr, last_idle, alpha, beta, gamma
	...
	if alloc_alg == 'MNTP':
		...
		for i in range(num_threads):
			if last_idle[i] != -1:
				...
				thr_list.append([i, len(alloc_queue[i]) + 1])
	...
	# The MTET heuristic #
		...
		for i in range(num_threads):
			...
			if last_idle[i] == -1:
				task = exec_queue[i][len(exec_queue[i]) - 1]
				total_et += task.f_time - t
	...
	# The TMCD heuristic #
		...
		for i in range(num_threads):
			if last_idle[i] != -1:
				num_tasks = len(alloc_queue[i])
			else:
				num_tasks = len(alloc_queue[i]) + 1
			...
			if last_idle[i] == -1:
				task = exec_queue[i][len(exec_queue[i]) - 1]
				sum_et += task.f_time - t

			thr_list.append([i, alpha * num_tasks / total_num_tasks + beta * val_it + gamma * sum_et / total_et])
			...
			if thr_num == 0:
				...
				for i in range(len(task_stack)):
					...
					if task_stack[i].dep == None or func.check_dep(task_list, task_stack[i].dep) == True:
						...
						thread_id = -1
						# Select empty queue belonging to an idle thread #
						for j in range(num_threads):
							if len(alloc_queue[j]) == 0 and last_idle[j] != -1:
								thread_id = j
								break
						# Select the queue using the allocation heuristic #
						if thread_id == -1:
				...
				for k in range(len(remove_list)):
					task_stack.remove(remove_list[k])
...
def execute(num_tasks, num_threads, task_list, deadline, alloc_alg, disp_alg, graphic_result):
	...
	func.export_scheduling(num_threads, exec_queue, 'improved heuristic', alloc_alg, disp_alg)

	# Draw the graphical output #
	if graphic_result == 1:
		func.graphic_result(num_threads, exec_queue, t, 'improved heuristic', alloc_alg, disp_alg)
